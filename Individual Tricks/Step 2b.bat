...
powercfg /delete 99999999-9999-9999-9999-999999999991

REM Define GUIDs for the base and new power schemes
set "BASE_SCHEME=e9a42b02-d5df-448d-aa00-03f14749eb61"
set "NEW_SCHEME=99999999-9999-9999-9999-999999999991"
...
powercfg -changename %NEW_SCHEME% "Extreme Performance 2.0" "Max performance on AC, efficient on battery"
...
REM === CORE PARKING - FULLY DISABLED VIA REGISTRY ===
REM This ensures ALL cores are unparked at the registry level, similar to Unpark CPU apps

REM First set via power plan
powercfg -setacvalueindex %NEW_SCHEME% SUB_PROCESSOR CPMINCORES 100 >nul 2>&1
powercfg -setacvalueindex %NEW_SCHEME% SUB_PROCESSOR CPMAXCORES 100 >nul 2>&1
powercfg -setdcvalueindex %NEW_SCHEME% SUB_PROCESSOR CPMINCORES 100 >nul 2>&1
powercfg -setdcvalueindex %NEW_SCHEME% SUB_PROCESSOR CPMAXCORES 100 >nul 2>&1
...
REM Now apply registry changes to FORCE unpark all cores (what Unpark CPU apps do)
...
REM Apply to all power schemes in registry to ensure it sticks
for %%G in (381b4222-f694-41f0-9685-ff5bb260df2e 8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c %NEW_SCHEME%) do (
    REM Set ValueMax to 0 (disables parking limit)
    reg add "HKLM\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\54533251-82be-4824-96c1-47b60b740d00\0cc5b647-c1df-4637-891a-dec35c318583\%%G" /v "ValueMax" /t REG_DWORD /d 0 /f >nul 2>&1
    
    REM Set core parking min to 100% (all cores)
    for /L %%i in (0,1,3) do (
        reg add "HKLM\SYSTEM\CurrentControlSet\Control\Power\Profile\%%i\%%G\54533251-82be-4824-96c1-47b60b740d00\0cc5b647-c1df-4637-891a-dec35c318583" /v "ACSettingIndex" /t REG_DWORD /d 100 /f >nul 2>&1
        reg add "HKLM\SYSTEM\CurrentControlSet\Control\Power\Profile\%%i\%%G\54533251-82be-4824-96c1-47b60b740d00\0cc5b647-c1df-4637-891a-dec35c318583" /v "DCSettingIndex" /t REG_DWORD /d 100 /f >nul 2>&1
    )
)

REM Also set in the main power settings location
